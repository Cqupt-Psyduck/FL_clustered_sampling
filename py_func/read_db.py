...
def create_MNIST_ds_1shard_per_client(n_clients, samples_train, samples_test):
    ...
    # 一共100个客户端，每个客户端只拥有一种数字。10个客户端一起拥有全部的一个数字的样本，每个客户端训练集500，测试集80
    for i in range(10):
...
def get_dataloaders(dataset, batch_size: int, shuffle=True):
    ...
    if dataset == "MNIST_iid":
        ...
        # 100个客户端，其中10、30、30、20和10个客户端分别有100、250、500、750和1000个训练样本，而测试样本占其训练规模的五分之一，所有的客户都考虑一批大小为50
        n_classes = 10
        n_clients = 100
